...
description: Mires's FastAPI and Celery integration pattern for Python repos. Use for start and status routes, durable processing or job rows, commit-before-enqueue behavior, scalar Celery task payloads, route-visible delay calls, task status polling, and keeping FastAPI request state out of workers.
---
