...
description: Mires's Alembic migration patterns for Python Postgres repos. Use for autogenerate output, async env.py configuration, settings-driven database URLs, pgvector extension migrations, downgrade safety, migration Makefile targets, and model-to-migration consistency.
---
...
For model design rules, use `patterns` first. For local infrastructure needed to run migrations, use `docker-compose-patterns`.
