---
name: patterns
description: Mires's Postgres and pgvector data modeling patterns for Python repos. Use for SQLAlchemy 2.x models, asyncpg Postgres URLs, pgvector embeddings, processing status tables, indexes, relationships, transactions, and database-backed application state.
---
...
For FastAPI engine/session wiring, use `postgres-connection-setup`. For schema migrations, use `alembic-patterns`.

## Convention Priority

Run project-conventions discovery before applying this skill to an existing repository. Existing ORM style, sync versus async access, model base classes, migration pattern, settings source, and transaction boundaries override the examples below. Treat source repo signals as examples only when they match the target repository.
