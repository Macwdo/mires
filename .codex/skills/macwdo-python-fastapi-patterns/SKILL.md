---
name: macwdo-python-fastapi-patterns
description: Macwdo's FastAPI feature patterns for Python repos. Use for route modules, dependency injection, Pydantic schemas, async SQLAlchemy sessions, router registration, health endpoints, upload/start/status endpoints, service delegation, and HTTP error handling in ml-products-grouping.
---
...
For narrower setup-only work, prefer `macwdo-python-fastapi-app-setup`. For endpoints that enqueue Celery jobs, also use `macwdo-python-fastapi-celery-integration`.
