...
- **WHEN** a user asks for React, React Query, React Hook Form, or Zod work
- **THEN** Mires routes the request through `.ai/agents/mires-frontend`
- **AND** it selects `mires-react` or the most specific nested React subagent for the task
...
The system SHALL keep granular implementation guidance outside the default public agent surface unless a compatibility redirect is required.
...
- **AND** the redirect remains available through the Codex-compatible `.codex/skills` export

### Requirement: Routing surface is verified
The project SHALL provide a validation path that detects accidental growth of the public Mires agent surface and drift between canonical `.ai` assets and Codex-compatible exports.
...
- **AND** it fails when required `.codex/skills` compatibility exports are missing or stale
