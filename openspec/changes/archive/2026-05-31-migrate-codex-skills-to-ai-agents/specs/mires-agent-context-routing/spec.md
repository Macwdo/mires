...
### Requirement: Retired Codex skill domains have `.ai` ownership
The system SHALL preserve useful Mires behavior from the retired Codex skill corpus under canonical `.ai` agents, skills, or references before removing Codex runtime assets.
...
- **AND** any retained legacy `.ai/skills` entry redirects to the canonical agent or reference owner
...
- **AND** routing does not require `.codex` assets
...
- **AND** it selects the most specific nested Python, FastAPI, or Django subagent for the task
...
- **AND** it selects `mires-react`, `mires-typescript-next`, or the most specific nested frontend subagent for the task
...
The system SHALL keep granular implementation guidance outside the default public skill surface unless a concise `.ai/skills` redirect is intentionally retained.
...
- **WHEN** a user or prompt invokes a legacy granular Mires skill ID that remains for `.ai` compatibility
...
- **AND** the redirect exists only in canonical `.ai/skills`

### Requirement: Routing surface is verified
The project SHALL provide a validation path that detects accidental growth of the public Mires agent surface, missing migrated domain ownership, and reintroduction of Codex runtime assets.
...
- **AND** it fails when active `.codex` runtime assets are present
