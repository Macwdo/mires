...
For tests that must prove `.delay()` or `.apply_async()` reaches a real broker and worker, use `celery-live-worker-testing` instead.
...
4. Narrow integration tests without a live worker
   ...
   - If the worker must consume from the broker, switch to `celery-live-worker-testing`.
