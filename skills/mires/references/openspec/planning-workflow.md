...
description: "Plan implementation work using the current Mires subagents and skills. Use when the user asks for an implementation plan, feature plan, or architecture plan."
---

# Mires Planner

Use this reference to produce implementation plans grounded in the repository and the relevant Mires pattern skills. This is a planning reference, not an implementation guide.
...
3. Start with `explorer` when the relevant surface is not obvious.
4. Verify external libraries, APIs, setup commands, architecture options, and current behavior from primary sources before finalizing the plan.
5. Name the narrow skills and references the implementation should use.
...
- Generic Python services: `backend` plus `python`.
