...
description: Mires's Docker Compose patterns for Python service repos. Use for docker-compose-dev.yml, Postgres pgvector, LocalStack SQS, MinIO, one-shot provisioning services, healthchecks, persistent volumes, port mappings, Makefile wiring, and local API or Celery worker service additions.
---
...
For deeper local infrastructure examples, use `docker-compose-local-infra`.
